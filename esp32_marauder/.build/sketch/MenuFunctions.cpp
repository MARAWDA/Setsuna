#line 1 "C:\\Users\\wisht\\New folder\\Setsuna\\esp32_marauder\\MenuFunctions.cpp"
...
void MenuFunctions::main(uint32_t currentTime)
{
  ...
  // Simple, deterministic C5 touch nav: tap a visible menu row to select/run it,
  // and keep the bottom bar as a reliable fallback for UP/OK/DN/BACK.
  // This avoids the ambiguity of earlier quarter-wide swipes and makes the
  // interaction feel consistent even with the board's touch calibration drift.
  #if defined(MARAUDER_C5_TOUCH_LCD_28)
  {
    ...
    if (pressed) {
      if (!touch_active) {
        ...
        bool idle_menu = ((wifi_scan_obj.currentScanMode == WIFI_SCAN_OFF) ||
                          (wifi_scan_obj.currentScanMode == WIFI_CONNECTED));
        if (idle_menu && current_menu && current_menu->list && current_menu->list->size() > 0) {
          const uint16_t row_top = KEY_Y - (KEY_H / 2);
          const uint16_t row_height = KEY_H + KEY_SPACING_Y;
          const uint16_t bottom_bar_y = TFT_HEIGHT - 38;

          if (last_y >= bottom_bar_y) {
            uint16_t quarter = TFT_WIDTH / 4;
            int list_size = current_menu->list->size();
            if (last_x < quarter) {
              current_menu->selected = (current_menu->selected == 0) ? (list_size - 1) : (current_menu->selected - 1);
              this->displayCurrentMenu(menu_start_index, true);
            } else if (last_x < 2 * quarter) {
              current_menu->list->get(current_menu->selected).callable();
            } else if (last_x < 3 * quarter) {
              current_menu->selected = (current_menu->selected + 1) % list_size;
              this->displayCurrentMenu(menu_start_index, true);
            } else if (current_menu->parentMenu != NULL) {
              this->changeMenu(current_menu->parentMenu, true);
            }
          } else {
            int row = (last_y - row_top) / row_height;
            int target_index = menu_start_index + row;
            if (target_index >= 0 && target_index < current_menu->list->size()) {
              current_menu->selected = target_index;
              this->displayCurrentMenu(menu_start_index, true);
              current_menu->list->get(target_index).callable();
            }
          }
        }
      }
    ...
    // Finger lifted: allow the next tap immediately.
    touch_active = false;
    last_x = 0;
    last_y = 0;
  }
